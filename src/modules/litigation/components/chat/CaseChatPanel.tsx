...
import type { CaseWithDocs } from "@/lib/types";
...
function inferCourtRegion(
  ...
  return court.replace(/(人民法院|法院|中级|高级|基层|互联网|知识产权|海事|金融)/g, "").trim() || court;
}
...
export function CaseChatPanel({
  ...
  const similarCasesHint = courtRegion
    ? `检索相似判例(按本案法院所在地 ${courtRegion} 优先),判断对我方诉求是支持还是不利 + 风险点,案例原文存进本地知识库(检索辅助,非法律意见)`
    : "检索相似判例(按本案法院所在地优先),判断对我方诉求是支持还是不利 + 风险点,案例原文存进本地知识库(检索辅助,非法律意见)";
