...
graph.case_id 必须等于当前案件。节点和关系 id 使用新生成的稳定 UUID，后续更新必须复用，不得按标题或日期重新计算。只使用白名单 node kind、edge kind、view kind 和 status。不得输出 HTML、脚本、远程 URL、图片地址或任意渲染器 option。layout 通常传空对象，由应用内 ELK/原生布局生成。

本工具只用于首次创建。若本案已经有工作区，不要重建或覆盖，应先读取现有结构并调用 propose_case_visual_update 保存更新提案，让用户在工作台逐项审阅。工具返回工作区 id、修订号和视图摘要后，只需告诉用户可进入可视化工作台查看、核对来源和继续编辑。
