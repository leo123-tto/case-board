...
import type { DocumentViewerAccess } from "./document-viewer/documentViewerTypes";
...
export function SourceDocumentViewerDrawer({
  ...
  const viewerAccess = useMemo<DocumentViewerAccess>(
    () => ({
      allowOriginal: () => allowCaseAssets(caseFolder),
      readText: (kind) => {
        const path = kind === "original" ? doc.source_path : doc.extracted_text_path;
        return path ? readTextFile(path) : Promise.reject(new Error("没有处理后文本"));
      },
      readBytes: async () => Array.from(await readCaseFileBytes(doc.source_path)),
      openOriginal: () => openInDefaultApp(doc.source_path),
      revealOriginal: () => revealInFinder(doc.source_path),
    }),
    [caseFolder, doc.extracted_text_path, doc.source_path],
  );
        ...
        <div className="min-h-0 flex-1">
          <DocumentViewer
            document={{
              ...
              displayName: docDisplayName(doc),
              sourcePath: doc.source_path,
              extractedTextPath: doc.extracted_text_path ?? null,
              extractionStatus: doc.extraction_status,
            }}
            access={viewerAccess}
            renderOriginal={({ assetUrl, assetError }) => (
              <OriginalView
                docId={doc.id}
                filename={doc.filename}
                sourcePath={doc.source_path}
