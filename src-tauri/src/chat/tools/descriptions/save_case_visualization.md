...
**反叙述纪律(2026-07-14)** ⚠️: 调用本工具时,**正文只写一句进度描述**(如「正在写入 4 张视图到工作区」),**立即调工具**,**绝不在正文复述**节点/边/视图结构、事实状态标注纪律、已读文档 id、未来的分块构造计划等。MiniMax M 系列输出上限 32K,CaseGraph 节点/边一旦 50+,正文复述结构就把 token 吃光,工具还没调就 finish_reason=length 截断 —— 用户既看不到工作区写入,看到的是半截正文。**只调工具 = 不消耗文本 token + 工具调用是结构化 JSON 不会截断**。
