-- 2026-07-20 · 修复少量升级库「0045 已记录但 AI 工作区表未落下」的迁移漂移。
-- 只增不改：健康库全部 no-op；缺表或部分落表的库补齐缺失 schema。
...
CREATE INDEX IF NOT EXISTS idx_ai_workspaces_active_updated
    ON ai_workspaces(archived_at, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_workspaces_recent
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ai_workspace_source_path_active
    ON ai_workspace_documents(workspace_id, normalized_source_path)
    WHERE kind = 'source' AND archived_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_ai_workspace_documents_active
    ON ai_workspace_documents(workspace_id, kind, archived_at, updated_at DESC);

CREATE TABLE IF NOT EXISTS ai_workspace_document_chunks (
    id            TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_chunks_document_page
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_versions_document
    ON ai_workspace_document_versions(document_id, version_no DESC);

CREATE TABLE IF NOT EXISTS ai_workspace_conversations (
    id              TEXT PRIMARY KEY,
    workspace_id    TEXT NOT NULL,
    title           TEXT NOT NULL CHECK (length(trim(title)) > 0),
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_conversations_active
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_messages_conversation
    ON ai_workspace_messages(conversation_id, created_at, id);

CREATE TABLE IF NOT EXISTS ai_workspace_tasks (
    id                   TEXT PRIMARY KEY,
    workspace_id         TEXT NOT NULL,
    conversation_id      TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_tasks_scope
    ON ai_workspace_tasks(workspace_id, conversation_id, status, updated_at DESC);

CREATE TABLE IF NOT EXISTS ai_workspace_document_proposals (
    id                   TEXT PRIMARY KEY,
    workspace_id         TEXT NOT NULL,
    document_id          TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_ai_workspace_proposals_document
