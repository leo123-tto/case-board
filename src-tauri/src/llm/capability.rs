...
impl ProviderCapability {
    ...
    /// Kimi Coding Plan 当前只接受 temperature=1；其余后端保留调用方设置。
...
mod tests {
    ...
    #[test]
    fn kimi_preset_keeps_agent_tool_support() {
        let capability = ProviderCapability::from_backend(
            "kimi",
            "https://api.kimi.com/coding/v1/chat/completions",
            "kimi-for-coding",
        );
        assert_eq!(capability.kind, LlmProviderKind::KimiCompat);
        assert!(capability.supports_tools);
        assert!(capability.requires_reasoning_replay_for_tool_calls);
        assert_eq!(capability.normalize_temperature(0.3), 1.0);
        assert_eq!(capability.normalize_temperature(0.0), 1.0);
    }
}
